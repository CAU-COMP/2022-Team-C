...
async function selectNewNote(connection, userId){
    const selectNewNoteQuery = `
        SELECT M2.name, C.course_name as course, N.explanation
...
async function selectPopularNote(connection){
    const selectPopularNoteQuery = `
        SELECT M.name, C.course_name as course, N.explanation
...
async function selectNote(connection, noteId) {      // 필기 쓴 유저 이름, 과목명, 업로드 일자, 포인트, 설명
    const noteInfoQuery = `
                  SELECT N.course_id, N.year, N.semester, N.instructor_name, N.upload_time, N.file_url, N.point, N.explanation, N.hits, 
